...
3. Pause the existing API, manual/monthly ingestion, discovery workers, and scheduler. Remove every `${{PostGIS.DATABASE_URL}}`/owner reference from those services before any can restart; a service must remain stopped until its restricted role is ready.
4. Generate different URL-safe 32+ character passwords for `seekandscore_api` and `seekandscore_ingestion`. Build sealed private-network runtime URLs for both LOGIN names; never print them in logs or shell history.
5. Create the private one-shot `db-migrate` service from `/infra/railway/db-migrate.example.toml`. It alone receives `MIGRATION_DATABASE_URL=${{PostGIS.DATABASE_URL}}`, both generated passwords, and both sealed runtime URLs. It has no public domain.
6. Deploy `db-migrate` at the exact release SHA. Require `release apply` and the subsequent `release verify` to report the same Alembic head and passing API/ingestion role audits. A failure blocks every later service deploy; concurrent releases fail on the database advisory lock.
7. Deploy API from `/infra/railway/api.example.toml` with only `DATABASE_URL=<sealed seekandscore_api URL>`, then deploy web from `/infra/railway/web.example.toml`. Do not attach `MIGRATION_DATABASE_URL`, the PostGIS owner URL, or ingestion credentials to API. Verify API startup role audit and `/readyz`; never add a public API/database domain.
8. Deploy the manual acquisition proof from `/infra/railway/ingestion-travis.example.toml`. Give it only `DATABASE_URL=<sealed seekandscore_ingestion URL>` plus `raw-artifacts` references; give it no owner/API database URL, public domain, Redis, outreach, alert-provider, auth, or web secrets. This template intentionally has no cron.
9. First deploy ingestion with `APP_ENV=staging`, `DATASET_MODE=live`, `INGESTION_ENABLED=false`, `INGESTION_SOURCE_ID=travis_tcad_parcels`, `INGESTION_RUN_PROFILE=proof`, exact page/max records `2`/`2`, exact cities `DEL VALLE,MANOR`, and no activation ID. Require the ingestion role audit to pass, then confirm acquisition fails closed without a network request.
10. From an authorized operator connection, prove `seekandscore_api` cannot create tables or insert/update/delete revision/audit rows, and `seekandscore_ingestion` cannot access `deal`/`platform` or create tables. Do not proceed if either runtime owns an object, can assume another role, or has database/schema CREATE/TEMPORARY.
11. Reconfirm that stopped application services and shared-variable scopes contain no owner URL or alias. Record a defense-in-depth PostGIS owner-credential rotation for a controlled follow-up and update only `db-migrate` when performed; there is no leak evidence, and rotation need not block source activation after all old owner-bearing services are stopped/removed and both restricted-role audits pass.
12. Reference the recorded private-acquisition decision, then set `INGESTION_ENABLED=true` and the exact reviewed activation ID only on the acquisition service. Execute the two-record `proof` profile and replay it once. Require two fetched records, no quarantine, a content-addressed private raw artifact, and no duplicate normalized rows on replay; then disable ingestion and clear the activation ID.
...
14. Verify raw object durability/checksums, source-run profile and status, artifact lineage, exactly bounded request count, parser outcome, and replay idempotency. Exercise the kill switch and inspect logs for secret/query leakage.
15. Only after both proof and cohort replays pass, change the service config path to `/infra/railway/ingestion-travis.cron.example.toml`, retain the same restricted ingestion URL, `INGESTION_RUN_PROFILE=cohort`, page/max `250`/`1000`, exact cities, and the reviewed acquisition activation ID, then enable the monthly `17 9 2 * *` schedule. Railway cron is UTC, may start late, and skips a new run while the previous process remains active; the process must exit and close connections.
16. Keep live display off during ingestion validation. After the complete cohort and replay checks pass, protect the public web origin with the required single-operator access gate, keep the API private, and enable reference display with `LIVE_SOURCE_DISPLAY_APPROVAL_ID=SRC-TCAD-TNR-BOUNDED-DISPLAY-20260813-V1`. Keep export, redistribution, outreach, and provider alert delivery off. Enabling acquisition alone never grants those capabilities.
